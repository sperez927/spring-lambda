...
public class DatabaseStack extends Construct {
  ...
  public DatabaseStack(
    ...
    dbInstance
      .getConnections()
      .allowFrom(
        dbInstance.getConnections().getSecurityGroups().get(0),
        Port.tcp(5432)
      );
    dbInstance
      .getConnections()
      .allowFrom(stackProps.getApplicationSecurityGroup(), Port.tcp(5432));

    createDbSetupLambdaFunction(stackProps, dbInstance, dbSecret);
  }

  private SecurityGroup createDatabaseSecurityGroup(IVpc vpc) {
    SecurityGroup databaseSecurityGroup = SecurityGroup.Builder.create(
      this,
      "DatabaseSG"
    )
